...
router.post('/logout', function(req, res) {
	console.log('logout!! userID =' + req.param('userID'));
	if(req.param('userID')){
		console.log('Enter into userID =' + req.param('userID'));
		model.logoutUser({}, function(err, obj){
			if(err){
				console.log('Enter into if(err) = ' + err);
				res.status(400).send({error: 'Invalid user'});
			} else {
				console.log('Enter into else err = ' + err);
				res.send(obj);
			}
		});
	} else {
